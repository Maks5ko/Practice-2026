...
namespace task19research
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            const int totalWorkPerCommand = 10000;
            int[] quantumSizes = { 1, 2, 5, 10, 20, 50, 100, 200, 500 };
            const int repeats = 5;

            var results = new List<ExperimentResult>();

            Console.WriteLine("Исследование влияния кванта на производительность (задание 19)");
        ...
        static void SaveResults(List<ExperimentResult> results)
        {
            using (var writer = new StreamWriter("results.txt"))
            {
                ...
                {
                    writer.WriteLine($"{r.Quantum}: {r.TimeMs.ToString(CultureInfo.InvariantCulture)}");
                }
            }
        }
        ...
        static void PlotResults(List<ExperimentResult> results)
        {
            ...
            plt.Title("Зависимость времени выполнения от размера кванта (задание 19)");
